...
class DataRecorder:
    ...
    @staticmethod
    def _usage_row(label: str, usage: dict[str, float | int]) -> list[Any]:
        """Shape one internal counter snapshot for terminal presentation."""
        columns = ("chat_count", "prompt_tokens", "completion_tokens", "total_tokens")
        return [label, *(usage[key] for key in columns), f"{usage['cost']:.4f}"]
    ...
    def print_summary(self) -> None:
        ...
        styles = ["cyan", "magenta", *("blue" for _ in range(3)), "green"]
        RichPrinter.table(headers, rows, "Token Usage and Cost Summary", styles)
