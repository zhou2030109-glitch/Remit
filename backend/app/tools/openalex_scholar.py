...
class OpenAlexScholar:
    ...
    def _openalex_identity(self) -> tuple[dict[str, str], dict[str, str]]:
        """Return optional polite-pool parameters and a Remit-specific user agent."""
        params: dict[str, str] = {}
        if self.email:
            params["mailto"] = self.email
        if self.api_key:
        ...
        return params, {"User-Agent": f"RemitResearch/1.0{contact}"}
    ...
    async def _search_openalex(
        ...
        identity_params, headers = self._openalex_identity()
